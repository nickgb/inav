...
static void updateEstimatedTopic(uint32_t currentTime)
{
    ...
    if (newEPH <= posControl.navConfig->inav.max_eph_epv) {
        newEPH *= 1.0f + dt;
    }

    if (newEPV <= posControl.navConfig->inav.max_eph_epv) {
        newEPV *= 1.0f + dt;
    }
    ...
    if (isEstZValid || useGpsZPos || isBaroValid) {
        ...
        if (isBaroValid) {
            ...
            /* Adjust EPV */
            newEPV = updateEPE(posEstimator.est.epv, dt, posEstimator.baro.epv, posControl.navConfig->inav.w_z_baro_p);
        }
#endif
        ...
        if (useGpsZPos) {
            ...
            /* Adjust EPV */
            newEPV = updateEPE(posEstimator.est.epv, dt, MAX(posEstimator.gps.epv, gpsResidual[Z][0]), posControl.navConfig->inav.w_z_gps_p);
        }
    ...
    if (isGPSValid) {
        ...
        /* Adjust EPH */
        newEPH = updateEPE(posEstimator.est.eph, dt, MAX(posEstimator.gps.eph, gpsResidualMagnitude), posControl.navConfig->inav.w_xy_gps_p);
    }
    else {
    ...
    if (posControl.navConfig->inav.w_acc_bias > 0) {
        ...
        if (isGPSValid) {
            accelBiasCorr.V.X -= gpsResidual[X][0] * sq(posControl.navConfig->inav.w_xy_gps_p);
            accelBiasCorr.V.X -= gpsResidual[X][1] * posControl.navConfig->inav.w_xy_gps_v;
            accelBiasCorr.V.Y -= gpsResidual[Y][0] * sq(posControl.navConfig->inav.w_xy_gps_p);
            accelBiasCorr.V.Y -= gpsResidual[Y][1] * posControl.navConfig->inav.w_xy_gps_v;

            if (useGpsZPos) {
                accelBiasCorr.V.Z -= gpsResidual[Z][0] * sq(posControl.navConfig->inav.w_z_gps_p);
            }

            if (useGpsZVel) {
                accelBiasCorr.V.Z -= gpsResidual[Z][1] * posControl.navConfig->inav.w_z_gps_v;
            }
        ...
        if (isBaroValid && !isAirCushionEffectDetected) {
            accelBiasCorr.V.Z -= baroResidual * sq(posControl.navConfig->inav.w_z_baro_p);
        }
